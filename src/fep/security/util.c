...
{
	char PikMak[33],buf[100],op1,op2;
	int n;
...
//			  _3DES((unsigned char *)caSekKey1,(unsigned char *)tmk+8,(unsigned char *)mkey+8);
			  TripleDes((unsigned char *)caSekKey1,(unsigned char *)PikMak,(unsigned char *)PikMak1 );
...
//	      TripleDes((unsigned char *)mkey,(unsigned char *)PikMak,(unsigned char *)PikMak2 );
				dcs_debug(caSekKey1,16,"<%s,%d>caSekKey1 data",__FILE__,__LINE__);
				dcs_debug(caSekKey2,16,"<%s,%d>caSekKey2 data",__FILE__,__LINE__);
				dcs_debug(mkey,8,"<%s,%d>tmk data",__FILE__,__LINE__);
				dcs_debug(PikMak1,8,"<%s,%d>PikMak1 data",__FILE__,__LINE__);
				dcs_debug(PikMak2,8,"<%s,%d>PikMak2 data",__FILE__,__LINE__);
	...
	//			DES((unsigned char *)mkey,(unsigned char *)PikMak,(unsigned char *)PikMak2);
	      TripleDes((unsigned char *)mkey,(unsigned char *)PikMak,(unsigned char *)PikMak2 );
				dcs_debug(caSekKey1,16,"<%s,%d>caSekKey1 data",__FILE__,__LINE__);
				dcs_debug(caSekKey2,16,"<%s,%d>caSekKey2 data",__FILE__,__LINE__);
				dcs_debug(mkey,16,"<%s,%d>tmk data",__FILE__,__LINE__);
				dcs_debug(PikMak1,8,"<%s,%d>PikMak1 data",__FILE__,__LINE__);
				dcs_debug(PikMak2,8,"<%s,%d>PikMak2 data",__FILE__,__LINE__);
	...
	{
			dcs_debug(tmk,16,"tmk mdata");
			_3DES((unsigned char *)caSekKey2,(unsigned char *)tmk,(unsigned char *)mkey);
		  _3DES((unsigned char *)caSekKey2,(unsigned char *)tmk+8,(unsigned char *)mkey+8);
		  dcs_debug(mkey,16,"tmk data");
		  TripleDes((unsigned char *)caSekKey1,(unsigned char *)PikMak,(unsigned char *)PikMak1 );
		  TripleDes((unsigned char *)caSekKey1,(unsigned char *)PikMak+8,(unsigned char *)PikMak1+8 );
//		  DES((unsigned char *)caSekKey2,(unsigned char *)PikMak,(unsigned char *)PikMak1);
//			DES((unsigned char *)mkey,(unsigned char *)PikMak,(unsigned char *)PikMak2);
      TripleDes((unsigned char *)mkey,(unsigned char *)PikMak,(unsigned char *)PikMak2 );
      TripleDes((unsigned char *)mkey,(unsigned char *)PikMak+8,(unsigned char *)PikMak2+8 );
			dcs_debug(caSekKey1,16,"SekKey data");
			dcs_debug(caSekKey2,16,"tekKey data");
			dcs_debug(mkey,16,"tmk data");
  ...
  *nOut=2;
  ...
  else if( op2=='Y')
  {
	...
	memcpy(outBuf+*nOut,"0000000000000000",16);
	dcs_debug(PikMak,16,"<%s> PikMak mw",__FUNCTION__);
  TripleDes((unsigned char *)PikMak,(unsigned char *)"\x00\x00\x00\x00\x00\x00\x00\x00",(unsigned char *)buf);
	...
	bcd_to_asc(outBuf+*nOut,buf,16,0);
	*nOut=*nOut+16;
  outBuf[0]=(*nOut-2)/256;
  outBuf[1]=(*nOut-2)%256;
...
/*
功能	用ANSI X9.19 MAC算法对数据做MAC。
*/
int DesGetMac(char *inBuf,int nIn,char *outBuf,int *nOut)
{
  int i,n;
	unsigned char buf[17],caTmpBuf[256],caMac[48];
	...
	if( inBuf[6]=='Y')
	{
		memcpy(PikMak,inBuf+1+5+1,32);
		memcpy(datalen,inBuf+1+5+1+32,4);
		memcpy(mdata,inBuf+1+5+1+32+4,atoi((char *)datalen));
  }
  else if( inBuf[6]=='X')
  {
  ...
  {
  	dcs_debug(0,0,"<DesGetMac> type valid[%02x]",inBuf[5]);
		return -1;
  }
	GetMastKey((char *)caSekIndex,(char *)caSekKey);
	...
	asc_to_bcd(caSekKey,caSekKey,32,0);
	asc_to_bcd(PikMak,PikMak,32,0);
	
	_3DES((unsigned char *)caSekKey,(unsigned char *)PikMak,(unsigned char *)mkey);
	_3DES((unsigned char *)caSekKey,(unsigned char *)PikMak+8,(unsigned char *)mkey+8);
	
	dcs_debug(caSekKey,16,"caSekKey data");
	dcs_debug(mkey,16,"mkey data");
	...
	if( inBuf[0]=='3')
	{
		...
		{
			for( n=0;n<8;n++){
			  caMac[n]=caMac[n]^mdata[i*8+n];
			}
		}
		dcs_debug(caMac,8," xor data result ");
		memset(caTmpBuf,0,sizeof(caTmpBuf));
		if( inBuf[6]=='Y')
		  TripleDes(mkey,caMac,caTmpBuf);	
		else
  	...
  	for( i=0; i<atoi((char *)datalen)/8; i++)
		{
			...
			dcs_debug(mdata+i*8,8,"<%s>  data ",__FUNCTION__);
			dcs_debug(caMac,8,"<%s> xor data result ",__FUNCTION__);
			if( inBuf[6]=='Y')
		    TripleDes(mkey,caMac,caTmpBuf);
		  else 
		  	DES(mkey,caMac,caTmpBuf);
		   memcpy(caMac,caTmpBuf,8);	
		   dcs_debug(caMac,8,"<%s> DES data result ",__FUNCTION__);
			//
		}
  }
  else
  {
  	 dcs_debug(0,0,"<DesGetMac> valid 1byte[%02x]",inBuf[0]);
		 return -1;
  }
//	bcd_to_asc((unsigned char*)buf,(unsigned char*)caTmpBuf,16,0);
  dcs_debug(caTmpBuf,8,"mac");
  *nOut=2;
	...
	*nOut=*nOut+2;
	...
	memcpy(outBuf+*nOut,"00",2);
	*nOut= *nOut+2;
  bcd_to_asc(caMac,caTmpBuf,16,0);
	memcpy(outBuf+*nOut,caMac,16);
	*nOut=*nOut+16;
...
{
	unsigned char buf[17],type1,type2;
	...
	unsigned char PinKey1[33],mKey1[33],PinKey2[33],mKey2[33],pan1[17],pan2[17],block1[4],block2[4];
	...
	memcpy(caSekIndex2,inBuf+i,5);i=i+5; type1=inBuf[i];i=i+1;
	if( type1=='X' )
	{
		memcpy(PinKey1,inBuf+i,16);i=i+16;
	}
	else
	{
		memcpy(PinKey1,inBuf+i,32);i=i+32;
	}
	type2=inBuf[i];
	i=i+1;
	if( type2=='X')
	{
		memcpy(PinKey2,inBuf+i,16);i=i+16;
	}
	else
	{
		memcpy(PinKey2,inBuf+i,32);i=i+32;
	}
	memcpy(block1,inBuf+i,2);
	memcpy(block2,inBuf+i+2,2);
	i=i+4;
	memcpy(SourcePinBlock,inBuf+i,16);i=i+16;
	if( memcmp(block1,"01",2)==0)
	{
		memcpy(pan1,inBuf+i,16); i=i+16;
  }
  if( memcmp(block2,"01",2)==0)
  {
		memcpy(pan2,inBuf+i,16); i=i+16;
  }
	GetMastKey((char *)caSekIndex1,(char *)caSekKey1);
	GetMastKey((char *)caSekIndex2,(char *)caSekKey2);
	...
	asc_to_bcd(caSekKey1,caSekKey1,32,0);
	asc_to_bcd(caSekKey2,caSekKey2,32,0);
	asc_to_bcd(PinKey1,PinKey1,32,0);
	asc_to_bcd(PinKey2,PinKey2,32,0);
	asc_to_bcd(SourcePinBlock,SourcePinBlock,32,0);
	
	_3DES((unsigned char *)caSekKey1,(unsigned char *)PinKey1,(unsigned char *)mKey1);
	_3DES((unsigned char *)caSekKey1,(unsigned char *)PinKey1+8,(unsigned char *)mKey1+8);
	_3DES((unsigned char *)caSekKey2,(unsigned char *)PinKey2,(unsigned char *)mKey2);
	_3DES((unsigned char *)caSekKey2,(unsigned char *)PinKey2+8,(unsigned char *)mKey2+8);
	
	dcs_debug(caSekKey1,16,"caSekKey1 data");
	dcs_debug(PinKey1,16,"PinKey1 data");
	dcs_debug(mKey1,16,"mKey1 data");
	dcs_debug(caSekKey2,16,"caSekKey2 data");
	dcs_debug(PinKey2,16,"PinKey2 data");
	dcs_debug(mKey2,16,"mKey2 data");
	if( type1 == 'X')
		 _DES((unsigned char *)mKey1,(unsigned char *)SourcePinBlock,(unsigned char *)mPinkBlock);
	else
		_3DES((unsigned char *)mKey1,(unsigned char *)SourcePinBlock,(unsigned char *)mPinkBlock);
	dcs_debug(mPinkBlock,8,"pan1=[%s]",pan1);
	asc_to_bcd(pan1,pan1,16,0);
	
	
	for( i=0 ;i <8 ;i++)
	 mPinkBlock[i]= mPinkBlock[i]^ pan1[i];
	dcs_debug(mPinkBlock,8,"pan2=[%s]",pan2);
	asc_to_bcd(pan2,pan2,16,0);
	dcs_debug(pan2,8,"pan2 bcd");
	for( i=0 ;i <8 ;i++)
	 mPinkBlock[i]= mPinkBlock[i]^ pan2[i];
	dcs_debug(mPinkBlock,8,"PinBlock");
	if( type2 =='X')
		DES((unsigned char *)mKey2,(unsigned char *)mPinkBlock,(unsigned char *)DestPinBlock);
	else
		TripleDes((unsigned char *)mKey2,(unsigned char *)mPinkBlock,(unsigned char *)DestPinBlock);
...
int DecTrack2(char *inBuf1,int nIn,char *outBuf,int *nOut)
{
	...
	char buf[1024],caTmpBuf[1024],type;
	...
	memcpy(caIndex,inBuf + i, 5);i=i+5;i=i+1;
	type=inBuf[5];
	dcs_debug(0, 0, "key type=[%c][%02x]",type,type);
	if( type == 'X')
	{
		memcpy(Tdk,inBuf+i,16);i=i+16;
	}
	...
	{	
		memcpy(Tdk,inBuf+i,32);i=i+32;
  }
	memcpy(datalen,inBuf+i,4);i=i+4;
		dcs_debug(0, 0, "datalen=[%s]",datalen);
	memcpy(Trackdata,inBuf+i,atoi((char *)datalen));
	...
	asc_to_bcd(caKey,caKey,32,0);
	asc_to_bcd(Tdk,Tdk,32,0);
	dcs_debug(caKey,16,"mastkey data");
	dcs_debug(Tdk,16,"Tdk data");
	_3DES((unsigned char *)caKey,(unsigned char *)Tdk,(unsigned char *)mkey);
	_3DES((unsigned char *)caKey,(unsigned char *)Tdk+8,(unsigned char *)mkey+8);
	...
	for( i=0; i<atoi((char *)datalen)/8; i++)
	{
      ...
      if(inBuf1[0]==0x30)
        	...
        	TripleDes(mkey,Trackdata+i*8,buf+i*8);
      else
			if(type == 'X')
				_DES(mkey,Trackdata+i*8,buf+i*8);
			else 
				_3DES(mkey,Trackdata+i*8,buf+i*8);
				
		//bcd_to_asc((unsigned char*)buf+i*16,(unsigned char*)caTmpBuf+i*8,16,0);
		dcs_debug(buf,i*8,"track  data");
	}
