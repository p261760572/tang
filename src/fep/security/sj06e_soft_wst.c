...
int appProc(char *inBuf,int nIn,char *outBuf,int *nOut)
{
	...
	if ( memcmp(inBuf,"1A",2)==0) /*生成随机密钥*/
		 return DesGetKey(inBuf+2,nIn-2,outBuf,nOut);
	else if ( memcmp(inBuf,"\x04\x20",2)==0) /*生成终端主密钥TMK*/
		 return DesGetTmk(inBuf+2,nIn-2,outBuf,nOut);
	else if ( memcmp(inBuf,"\x04\x21",2)==0) /*生成数据密钥PIK/MAK*/
		 return DesGetPikMak(inBuf+2,nIn-2,outBuf,nOut);
  else if ( memcmp(inBuf,"\xc0\x49",2)==0) /*转加密key*/
		 return DesToRsaKey(inBuf+2,nIn-2,outBuf,nOut);
	else
  {
