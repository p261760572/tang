...
int           off , len , k,msgtypelen ;
  ...
  if ( iso == NULL)
  {
  	dcs_log(0,0,"<%s> iso is null",__FUNCTION__);
  	return -1;
  }
	if ( iso->iso8583 == NULL)
	{	
	 dcs_log(0,0,"<%s> iso->iso8583 is null",__FUNCTION__);
	     return -1;
	}
	if ( dstr == NULL )
	{
		dcs_log(0,0,"<%s> dstr is null",__FUNCTION__);
			 return -1;
	}
  if ( strlen <10) 
  {
  		dcs_log(0,0,"<%s> strlen < 10 [%d]",__FUNCTION__,strlen);
  		return -1;
  }
...
/*   dcs_debug(0,0,"Begin unpaket "); */
	...
	if ( iso->head_flag )
	{
		if ( iso->headlen >200  || iso->headlen < 0)
		{
			dcs_log(0,0,"<%s> iso->headlen >200  || iso->headlen < 0",__FUNCTION__);
				 return -1;
		}
		if( iso->headlen >strlen )
		{
				 dcs_log(0,0,"<%s> iso->headlen >strlen",__FUNCTION__);
				 return -1;
		}
