...
int gl_sock,role=0,stop=0,crypt_flag=0;
...
//	memset(posp_ip1,0,sizeof(posp_ip1));
  gs_run_mode = 0;
	memset(&g_para,0,sizeof(struct PARA));
	...
	memset(&gl_fd,0,sizeof sizeof(struct FD_REC)*FD_SETSIZE);
		...
		for ( i=1; i<argc ;i++)
		{
			 if(memcmp("svr_ip",argv[i],6)==0 )
			 {
			 	 if ( strlen( argv[i] ) >6)
			 	 {
			 	     sscanf(argv[i],"%*[^=]=%s",g_para.svr_ip);
			 	     dcs_debug(0,0,"svr_ip=%s !\n",g_para.svr_ip);
			   }
			   else 
			   {
			 ...
			 else if(memcmp("svr_port",argv[i],8)==0 )
			 {
			 	  if ( strlen( argv[i] ) >8 )
			 	  {
			 	  		sscanf(argv[i],"%*[^=]=%d",&g_para.svr_port);
			 	  		dcs_debug(0,0,"svr_port=%d !\n",g_para.svr_port);
			 	  }
			 	  else 
			   {
			   	  dcs_log(0,0,"error entry para=%s",argv[i]);
