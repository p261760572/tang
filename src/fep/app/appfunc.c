...
int BC_SendMessageToFolder( char *resp )
{
    ...
    {
        iRc = fold_write(iDestFolderId, gs_myFid, g_folder_msg.data, nbytes);
        if ( iRc < 0 )
        {
...
int BC_SendMonitor_trans(char *transflag,char *sendflag,tl_bc_trans_log_def *p_BcTransLog, ti_bc_trans_inf_def *p_tiBcTransInf)
{
    ...
    *(g_Moni_Data.data + sizeof(g_Moni_Data.data) -1) = 0;
    ...
    MEMCPY_THIS(ptr, p_BcTransLog_o->mchnt_cd, 15);
...
//        MEMCPY_THIS(ptr, "    ", 4);
//    }
	MEMCPY_THIS(ptr, p_tiBcTransInf->msg_tp, 4);
...
//错误信息入库
int ErrInfoInBcTransLog(char *fileName, int lineNo, char *resp_code, char *errInfo, int flag)
{ 
	...
	BC_SendExeception("03", "警告, %s", errInfo);
  	...
  	{
    	BC_SendExeception("03","警告, 记录异常表失败.流水号:[%s],终端号:[%s],商户号:[%s]",p_BcTransLog->sys_tra_no,p_BcTransLog->term_id,p_BcTransLog->mchnt_cd);
  	}
 	if(flag ==0 )return;
  	//记录入库
	...
	if(0 != iRet)
	{
		BC_SendExeception("03", "警告, 更新库失败！流水号:[%s],终端号:[%s],商户号:[%s]",p_BcTransLog->sys_tra_no,p_BcTransLog->term_id,p_BcTransLog->mchnt_cd);
	}
}

//加密保存信息卡信息
int EncryptInfo(char *cvv, char *vdate, char *encrypt_info)
{
	ti_bc_ins_inf_def tiBcInsInf;	
	ti_bc_ins_inf_def *p_tiBcInsInf=&tiBcInsInf;
	char e_vdate[17], e_cvv[17], tempKeyIndex[6];
	char resp_code[10];
	int len;
	if( 0 != BC_Search_ins_inf(resp_code, p_tiBcInsInf, "00005500"))
	{
		dcs_log(0, 0, "<FILE:%s,LINE:%d>查询机构00005500失败",__FILE__,__LINE__); 
		return -1;
	}
	...
	memcpy(tempKeyIndex+2, p_tiBcInsInf->key_index, 3);
	if( 1 != EncTrackPrg(resp_code, tempKeyIndex, p_tiBcInsInf->key_2, 8, vdate, &len, e_vdate))
	{
		dcs_log(0, 0, "<FILE:%s,LINE:%d>加密VDATA失败!",__FILE__,__LINE__); 
		return -1;
	}
	if( 1 != EncTrackPrg(resp_code, tempKeyIndex, p_tiBcInsInf->key_2, 8, cvv, &len, e_cvv))
	{
		dcs_log(0, 0, "<FILE:%s,LINE:%d>加密CVN失败!",__FILE__,__LINE__); 
		return -1;
	}
	memcpy(encrypt_info, p_tiBcInsInf->key_2, 16);
	bcd_to_asc(encrypt_info + 16, e_vdate, 16, 0);
	bcd_to_asc(encrypt_info + 32, e_cvv, 16, 0);
	return 1;
}

//解密信用卡信息
int decryptInfo(char *cvv, char *vdate, char *encrypt_info, ti_bc_ins_inf_def *p_tiBcInsInf, char *ins_id_cd, int flag)
{
	ti_bc_ins_inf_def tiBcInsInf;	
	char e_vdate[17], e_cvv[17], tmpkeyindex[6];
	char resp_code[10];
	int len;
	if(p_tiBcInsInf==0)
	{
		...
		if( 0!= BC_Search_ins_inf(resp_code, p_tiBcInsInf, ins_id_cd))
	  	...
	  	memcpy(p_tiBcInsInf->key_2, encrypt_info, 16);
	  	encrypt_info += 16;
	}
	memset(tmpkeyindex, 0 ,sizeof(tmpkeyindex));
	strcpy(tmpkeyindex, "S0");
	memcpy(tmpkeyindex+2, p_tiBcInsInf->key_index, 3);
	memset(e_vdate, 0, sizeof(e_vdate));
	memset(vdate, 0, sizeof(vdate));
	if(flag)
	{
		asc_to_bcd(e_vdate, encrypt_info, 16, 0);
		if(  1 != DecTrackPrg(resp_code, tmpkeyindex, p_tiBcInsInf->key_2, 8, e_vdate,   &len, vdate))
		{
			return -1;
		}
		memset(e_cvv, 0, sizeof(e_cvv));
		memset(cvv, 0, sizeof(cvv));
		asc_to_bcd(e_cvv, encrypt_info+16, 16, 0);
		if(  1 != DecTrackPrg(resp_code, tmpkeyindex, p_tiBcInsInf->key_2, 8, e_cvv,   &len, cvv))
		{
			return -1;
		}	
	}
	...
	{	
		if(  1 != DecTrackPrg(resp_code, tmpkeyindex, p_tiBcInsInf->key_2, 8, encrypt_info,   &len, vdate))
		{
			return -1;
		}
	}
