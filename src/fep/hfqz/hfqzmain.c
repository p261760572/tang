...
int process( char* srcBuf,int  iFid, int iLen )
{
	...
	else if(memcmp(process, "810001", 6) == 0) 
	{
		if(memcmp(msgtype, "0820", 4) == 0) //交易申请
		{
			ExpendApp(&iso);
		}
		else if(memcmp(msgtype, "0880", 4) == 0)//交易查询
		{
		}
		else
...
static int LoadConfig()
{
	...
	int flag;
	FILE *fd;
	strcpy(caBuffer, "config/hfqz.conf");
    ...
    dcs_debug(0, 0, "<FILE:%s,LINE:%d>\nbackEndUrl[%s]\nExpendUrl[%s]\nKeyIndex[%s]\nInstIdCode[%s]\n", __FILE__, __LINE__, BackEndUrl, ExpendUrl, KeyIndex, InstIdCode);
	return 1;
}
...
static int DoLoop()
{
    ...
    for ( ;; )
    {
		...
		{
			PayResultReply(g_Rcv_Buffer+4, iRead-4);
		}
		else
		{
...
static int CreateMyFolder()
{
    ...
    gs_myFid = fold_create_folder("HFQZ");
    if ( gs_myFid < 0 )
        gs_myFid = fold_locate_folder("HFQZ");
    if ( gs_myFid < 0 )
    {
...
int main(int argc, char *argv[])
{
    ...
    if ( 0 > OpenLog(argv[0]))
    {
        BC_SendExeception("04","<FILE:%s,LINE:%d>打开日志失败，hfqzapp进程启动失败，退出");
        appSrvExit(0);
    }
    dcs_log(0,0, "<FILE:%s,LINE:%d>hfqzapp Servers is starting up...\n",__FILE__,__LINE__);
    ...
    if ( CreateMyFolder() < 0 )
    {
        dcs_log(0, 0, "创建folder文件夹失败，hfqzapp进程启动失败，退出");
        appSrvExit(0);
    }
    ...
    dcs_log(0,0,"*************************************************\n"
            "!!      HFQZ Application Servers startup completed. !!\n"
            "*************************************************\n");
