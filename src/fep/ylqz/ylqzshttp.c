...
static void appSrvExit(int signo)
{
    ...
    dcs_log(0,0,"<FILE:%s,LINE:%d>YLQZSHTTP Server terminated.\n",__FILE__,__LINE__);
    DasEnd( COMMIT );
...
static void show_result(struct shttpd_arg *arg)
{
	...
	if(!strcmp(request_method, "POST"))
	{
		...
		dcs_debug(0, 0, "<FILE:%s,LINE:%d>xnxnxn\n request_method[%s]", __FILE__, __LINE__, request_method);
		memcpy(dst, "HTTP", 4);
...
/*static void show_getfileurl(struct shttpd_arg *arg)
{
	const char *request_method, *request_uri, *query_string;
	char dst[2048];
	int dst_len;
	request_method = shttpd_get_env(arg, "REQUEST_METHOD");
	request_uri = shttpd_get_env(arg, "REQUEST_URI");
	...
	if(!strcmp(request_method, "POST"))
	{
		...
		dst_len = _shttpd_url_decode(arg->in.buf, arg->in.len, dst, sizeof(dst));
	   	dcs_debug(0,0, "<FILE:%s,LINE:%d>result post read:[%d]%s!", __FILE__,__LINE__, arg->in.len, arg->in.buf);		
	}
	else if(query_string != NULL)
	{
	   	dcs_debug(0,0, "<FILE:%s,LINE:%d>result read:%s!", __FILE__,__LINE__, query_string);		
	}
	shttpd_printf(arg, "%s",
		"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n"
		"<html><body><h1>RECV_ORD_ID_0925112041\r\nReciver Ok!");
	arg->flags |= SHTTPD_END_OF_OUTPUT;
	return;
}*/
/*
...
int main()
{
    ...
    pthread_t sthread;
    int argc;
    char *argv[4] = {"ylqzshttp", "-ports", "8080", NULL};
    argc = 3;
    catch_all_signals(appSrvExit);
    if ( 0 > OpenLog("ylqzapp"))
    {
        BC_SendExeception("04","<FILE:%s,LINE:%d>打开日志失败，YLQZSHTTP进程启动失败，退出");
        appSrvExit(0);
    }
    dcs_log(0,0, "<FILE:%s,LINE:%d>YLQZSHTTP Servers is starting up...\n",__FILE__,__LINE__);
    ...
    gs_appSvrFid = fold_locate_folder("YLQZ");
    if ( gs_appSvrFid < 0 )
    {
    ...
    dcs_log(0,0,"*************************************************\n"
            "!!      YLQZSHTTP Application Servers startup completed. !!\n"
            "*************************************************\n");
...
//	shttpd_set_option(ctx, "ports", "9880");
	shttpd_register_uri(ctx, "/result", &show_result, NULL);
//	shttpd_register_uri(ctx, "/getfileurl", &show_getfileurl, NULL);
	...
	for (;;)
	{
		...
		if(0 > fold_locate_folder("YLQZ"))break;
	}
