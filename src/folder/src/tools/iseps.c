...
{  
    ...
    for(i=0; i <  MAX_PROCS;)
    {
        ...
        {
            //see if this procname registered already
            for(j=0; j < i; j++)
                if(strcmp(g_procTbl[j].procName, pProcName) == 0)
                    break;

            if( j < i) //registered already
                g_procTbl[j].procInstances += atoi(pMin);
            else
            {
...
int IsConfiguredProc(char *psLine)
{
    ...
    for(i=0; i <  g_iProcs; i++)
    {
        ...
        //added by yhc 2000.8.10 begin.
        if(strstr(psLine, "ism.x"))
        {
        	g_procTbl[i].procAlive ++;
        	return 1;
        }
        //added by yhc 2000.8.10 end.
    }
...
int printTitle(FILE *fpOut)
{
    //print the title
    fprintf(fpOut, "       ");
    fprintf(fpOut, "%s", RPAD("ProcessName",SPACE, 20));  //command line
    fprintf(fpOut, "%s", RPAD("PID",        SPACE, 8));   //pid
    fprintf(fpOut, "%s", RPAD("PPID",       SPACE, 8));   //ppid
    fprintf(fpOut, "%s", RPAD("StartTime",  SPACE, 12));  //start time
    fprintf(fpOut, "%s", RPAD("RunElapse",  SPACE, 8));   //running elapse
    fprintf(fpOut, "\n"); 
}

int printLine(FILE *fpOut)
{
    fprintf(fpOut, "%s", RPAD("-",  '-', 62));
    fprintf(fpOut, "\n"); 
    fflush(fpOut);
}

int printStat(FILE *fpOut)
{
    int i, delta;
    int isOkay = 1;
    ...
    fprintf(fpOut, "%d processes predefined, %d processes running.\n",
                     sumCfg, sumRun);
#endif
    ...
    for(i=0; i < g_iProcs; i++)
    {
        ...
        if(delta > 0)
        {
            fprintf(fpOut, "warning: %3d %s died.\n",
                             delta,
    ...
    if(isOkay)
        fprintf(fpOut, "info: all processes running ok.\n");
    fflush(fpOut);
}
